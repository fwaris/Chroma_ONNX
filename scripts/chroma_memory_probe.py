...
def max_numeric_snapshot(snapshots: list[dict], key: str) -> float | None:
...
def main() -> int:
    ...
    snapshots["afterInputs"] = memory_snapshot(torch)
    ...
    snapshots["afterModelLoad"] = memory_snapshot(torch)
    ...
    snapshots["afterGenerate"] = memory_snapshot(torch)
    ...
    report = {
        "backend": "python",
        "mode": "python_chroma_persistent_memory_probe",
        "device": device,
        ...
        "memory": {
            **snapshots,
            "peakPrivateGb": max_numeric_snapshot(snapshot_values, "privateGb"),
            "peakWorkingSetGb": max_numeric_snapshot(snapshot_values, "workingSetGb"),
            "peakGpuAllocatedGb": max_numeric_snapshot(snapshot_values, "gpuMaxAllocatedGb"),
            "peakGpuReservedGb": max_numeric_snapshot(snapshot_values, "gpuMaxReservedGb"),
        },
        "wav": wav,
        "artifacts": {
    ...
    (output_dir / "memory_report.json").write_text(json.dumps(report, indent=2), encoding="utf-8")
